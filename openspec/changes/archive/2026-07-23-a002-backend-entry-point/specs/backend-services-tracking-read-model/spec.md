...
- **THEN** el backend responde con HTTP `200`, `status=true` y `data` como colección de puntos con `latitude`, `longitude`, `created_at`
...
El backend SHALL exponer `GET /api/services/tracking` para devolver en una sola respuesta una colección de servicios con sus trackings anidados (relación 1:N), orientada a vistas globales de monitoreo.

#### Scenario: Consulta agregada de tracking exitosa
- **WHEN** un cliente autenticado consulta `GET /api/services/tracking`
- **THEN** el backend responde con HTTP `200`, `status=true` y `data` como colección de objetos de servicio con `id`, `name`, `start_time`, `end_time`, `polyline` y `trackings` (array de objetos con `latitude`, `longitude`, `created_at`)
...
- **THEN** emite una estructura homogénea de servicios con trackings anidados, compatible con el frontend sin depender del modelo interno
