# Capability: tracking-simulator-simulation

## Purpose

Definir el contrato funcional del microservicio `tracking-simulator` para generacion de servicios, construccion de rutas y simulacion continua de tracking compatible con el backend.
...
- **THEN** el simulador asigna automaticamente `start_time` y `end_time` segun su logica interna y no requiere que el backend los envie
