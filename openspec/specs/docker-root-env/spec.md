...
Provide a single root `.env.example` file containing all environment variables for the entire Docker stack, using prefixed variable names (BACKEND_*, SIMULATOR_*, MYSQL_*, FRONTEND_*, NGINX_*, COMPOSE_PROJECT_NAME, APP_ENV) as the source of truth for docker-compose interpolation.
